...
export default function CardDispositivoOneID({ item, onAlteraStatus}) {
    const [isEnabled, setIsEnabled] = useState(item.statusDispositivo == 1? true : false);
    const toggleSwitch = () => [setIsEnabled(previousState => !previousState), onAlteraStatus(item.codigoPin, item.statusDispositivo == 1? 2 : 1)];
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.containerBody}>
                <Text style={styles.textH2}>{item.cdDispositivo}</Text>
